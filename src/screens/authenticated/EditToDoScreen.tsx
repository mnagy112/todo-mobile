...
const EditToDoScreen = ({ route }: Props) => {
  ...
  const editedToDo: ToDo | undefined = route?.params?.id
    ? toDos.find((toDo) => toDo.id === editedToDoId)
    : undefined;
    ...
    return (
      <Layout>
        <View style={styles.heading}>
          ...
          <Button type="error" onPress={handlePressBack}>
            Back
          </Button>
        </View>
  ...
  return (
    <Layout>
      <View style={styles.heading}>
        <Text style={styles.title}>{editedToDoId ? 'Edit ToDo' : 'Create ToDo'}</Text>
        <Button type="error" onPress={handlePressBack}>
          Back
        </Button>
      </View>
      ...
      <Button type="success" onPress={handlePressSave} disabled={title === ''}>
        Save
      </Button>
    </Layout>
  );
